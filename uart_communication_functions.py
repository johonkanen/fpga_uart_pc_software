...
#-----------------------------------
def request_data_stream_from_address(address, number_of_registers):
    uart_message = 5
    uart_link.write(uart_message.to_bytes(1, "big"))
    uart_message = address
    uart_link.write(uart_message.to_bytes(2, "big"))
...
#-----------------------------------
def get_streamed_data(number_of_registers):
...
def stream_data_from_address(address, number_of_registers):
    request_data_stream_from_address(address, number_of_registers)
    return get_streamed_data(number_of_registers)
